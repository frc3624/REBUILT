...
public class SwerveSubsystem extends SubsystemBase
{
  ...
  public Command followAprilTagLimelight() {
    ...
    final double kTurnP = 0.03;     // rotation gain
    final double kDriveSpeed = 1.0; // meters/sec

    return run(() -> {
        if (!LimelightHelpers.hasTarget(LIMELIGHT_NAME)) {
        ...
        double rotation = MathUtil.clamp(-tx * kTurnP, -2.5, 2.5);

        drive(
            new Translation2d(kDriveSpeed, 0),
            rotation,
            false
        );
    }).withName("FollowAprilTagLimelight");
}
/**
 * Command to drive with manual translation but automatic rotation to AprilTag
 * @param translationX Translation in X direction from driver
 * @param translationY Translation in Y direction from driver
 * @return Command that allows manual driving with auto-rotation
 */
public Command driveWithAutoRotation(DoubleSupplier translationX, DoubleSupplier translationY) {
  ...
  return run(() -> {
      // Get driver's translation inputs
      ...
      // Calculate automatic rotation from Limelight
      double rotation = 0.0;
      if (LimelightHelpers.hasTarget(LIMELIGHT_NAME)) {
          double tx = LimelightHelpers.getTX(LIMELIGHT_NAME);
      ...
      Translation2d scaledTranslation = SwerveMath.scaleTranslation(
          new Translation2d(xSpeed, ySpeed), 0.8);
      
      // Drive with manual translation + auto rotation
      swerveDrive.drive(scaledTranslation, rotation, false, false);
  }).withName("DriveWithAutoRotation");
}

public Command driveToAprilTagDistance(double desiredTA) {
  final String LIMELIGHT_NAME = "limelight";

  final double kTurnP = 0.03;
  final double kDriveP = 0.6;
  ...
  return run(() -> {
      ...
      double rotation = MathUtil.clamp(-tx * kTurnP, -2.5, 2.5);
      double forward = MathUtil.clamp((desiredTA - ta) * kDriveP, -1.5, 1.5);

      // Stop condition
      if (Math.abs(tx) < 1.0 && Math.abs(desiredTA - ta) < 0.3) {
          drive(new Translation2d(0, 0), 0, false);
      } else {
...
// 3. Add this command to drive toward detected AprilTag:
/**
 * Command to drive toward the closest detected AprilTag
 * @return Command that drives toward AprilTag
 */

// 4. Add a more sophisticated version that stops at a certain distance:
/**
 * Command to drive to a specific distance from an AprilTag
